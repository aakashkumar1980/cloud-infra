...
locals {
  ...
  component_version = "v1_0"
  # Name suffix with test_ prefix to distinguish from production resources
  name_suffix_version = "${local.REGION_N_VIRGINIA}-${var.profile}-${local.tags_common["company"]}_${local.component_version}-${local.tags_common["managed_by"]}"
  name_suffix = "${local.REGION_N_VIRGINIA}-${var.profile}-${local.tags_common["company"]}-${local.tags_common["managed_by"]}"
}
